...
public class LogAspect {
    ...
    @Before("execution(public int com.lingfenglong.spring6.annotationaop.CalculatorImpl.*(..))")
    public void beforeMethod(JoinPoint joinPoint) {
        System.out.println("Logger 前置通知  " + joinPoint.getSignature().getName() + Arrays.toString(joinPoint.getArgs()));
    }

    @After("execution(* com.lingfenglong.spring6.annotationaop.CalculatorImpl.*(..))")
